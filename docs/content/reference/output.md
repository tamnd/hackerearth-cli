...
describes what users get. Pick a format with `-o`, or let he choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
he <command> -o table   # aligned columns for reading
he <command> -o jsonl   # one JSON object per line, for piping
he <command> -o json    # a single JSON array
he <command> -o csv     # spreadsheet friendly
he <command> -o tsv     # tab-separated
he <command> -o url     # just the URL column
he <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
he <command> --fields id,title,url
```
...
```bash
he <command>            # a table, because this is a terminal
he <command> | wc -l    # JSONL, because this is a pipe
```
